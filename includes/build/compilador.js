...
tinify.key = "fBGPi3-ePvtHFUpDOI5QxeqhqCWG48bD";
...
function initCompilacao() {
    ...
    function controlCSS() {
        del([infoBuild.path, infoCSS.path], function (err) {
            ...
            copy.each([infoCSS.cssPath], pasta, function (err) {
                ...
                copy.each([infoCSS.printPath], pasta, function (err) {
                    if (err) throw err;
                        controlJS();
                });
            })
        ...
        tinify.validate(function(err) {
            if (err) {
                try {
                    createZIP();
                    loaderServer();
                    console.log("Erro de validação do TinyPNG ou falta de net - img não comprimidas");
                }
                catch(err) {
                    createZIP();
                    loaderServer();
    ...
    function tinyPNG() {
        ...
        var _imgPATH  = pasta +"/includes/img";
        var imgArray = [];

        fs.readdir( _imgPATH , function (err, files) {
            //handling error
            if (err) {
            ...
            files.forEach(function (file) {
                ...
                if( String(file).indexOf(".png") > 1 ) imgArray.push( file );
                else if( String(file).indexOf(".PNG") > 1 ) imgArray.push( file );
                else if( String(file).indexOf(".jpg") > 1 ) imgArray.push( file );
                else if( String(file).indexOf(".JPG") > 1 ) imgArray.push( file );
                else if( String(file).indexOf(".jepg") > 1 ) imgArray.push( file );
                else if( String(file).indexOf(".JEPG") > 1 ) imgArray.push( file );
            });
            ...
            imgArray.forEach(function (file) {
                var source = tinify.fromFile(_imgPATH +"/"+ file);
                source.toFile(_imgPATH +"/"+ file , function(err) {
                    if (err) throw err;
