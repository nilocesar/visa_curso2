define(['jquery', 'jquery_scorm'], function ($) {
    ...
    var scorm = function () {
        ...
        $public.controlLessonLocationCurrent = function controlLessonLocationCurrent(_indice) {
            ...
            if( $public.getSuspendata("av") ){
                var _avArray = $public.getSuspendata("av").split('_'); 
                
                if( parseInt(_avArray[0]) < parseInt(_indice) ){
                    $public.setSuspendata("av" , _indice +"_"+ $parent.config[ _indice ].avaliada  );
                }
            }else{
                $public.setSuspendata("av" , _indice +"_"+ $parent.config[ _indice ].avaliada  );
            }
